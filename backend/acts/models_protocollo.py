...
class ProtocolloAttoNotarile(models.Model):
    ...
    # Relazioni (usando stringhe per evitare circular imports)
    ...
    def genera_protocollo_da_appuntamento(cls, appuntamento, created_by=None):
        ...
        with transaction.atomic():
            ...
            # Recupera il notaio (supporta sia campo nuovo che vecchio)
            notary = appuntamento.notary if hasattr(appuntamento, 'notary') and appuntamento.notary else None
            if not notary and hasattr(appuntamento, 'notaio') and appuntamento.notaio:
                # Fallback: recupera Notary dall'account Notaio vecchio
                # Assumiamo che ci sia una relazione tra Notaio (accounts) e Notary (notaries)
                # Se non c'è, solleva errore
                raise ValueError(
                    f"Appuntamento {appuntamento.id} usa il vecchio campo 'notaio' (accounts.Notaio). "
                    f"Aggiorna l'appuntamento per usare il campo 'notary' (notaries.Notary)."
                )
            
            if not notary:
                raise ValueError(f"Nessun notaio associato all'appuntamento {appuntamento.id}")
            
            # Raccogli dati parti coinvolte
            # Recupera il cliente dal partecipante richiedente
            richiedente = appuntamento.partecipanti.filter(ruolo='richiedente').first()
            cliente_nome = 'N/A'
            cliente_id = None
            cliente_email = appuntamento.created_by_email or 'N/A'
            ...
            if richiedente and richiedente.cliente:
                ...
                cliente_nome = f"{first_name} {last_name}".strip() or "Cliente"
                if hasattr(richiedente.cliente, 'user') and richiedente.cliente.user:
                    cliente_id = str(richiedente.cliente.user.id)
                    if hasattr(richiedente.cliente.user, 'email'):
                        cliente_email = richiedente.cliente.user.email
                else:
                    cliente_id = str(richiedente.cliente.id)
            ...
            parti_coinvolte = {
                ...
                'notaio': {
                    'id': str(notary.id),
                    'nome': notary.nome or '',
                    'cognome': notary.cognome or '',
                    'email': notary.user.email if hasattr(notary, 'user') and notary.user else 'N/A',
                },
